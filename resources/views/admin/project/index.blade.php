<x-admin-layout>
    <button class="btn py-2 px-4 dark:bg-white rounded-sm my-2 float-right"><a href="{{ route('project.create') }}">Create
            New
            Project</a></button>
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div role="list" class="grid grid-cols-3 gap-x-4 gap-y-8 sm:gap-x-6 lg:grid-cols-3 xl:gap-x-8">
            @if (!empty($projects))
                @foreach ($projects as $project)
                    <a href="{{ route('task.add', $project->id) }}">
                        <li
                            class="relative list-none bg-gray-200 border border-gray-300 dark:border-0 dark:bg-gray-500 rounded">
                            <div
